...
longest :: (Foldable ll, Ord a) => ll [a] -> [a]
longest xs = foldr1 longerOrSmallerFirst xs

longerOrSmallerFirst :: (Ord a) => [a] -> [a] -> [a]
longerOrSmallerFirst xs ys = case compare xsl ysl of
  EQ -> smallerFirst xs ys
  GT -> xs
  LT -> ys
  where
    xsl = length xs
    ysl = length ys
    smallerFirst (x : xs) (y : ys)
      | x < y = x : xs
      | otherwise = y : ys
...
incrementKey k ps = map incrementElem ps
  where
    incrementElem (k', v)
      | k == k' = (k', v + 1)
      | otherwise = (k', v)

-- incrementElem :: (Eq k, Num v) => k -> (k, v) -> (k, v)
-- incrementElem k p
--   | k == fst p = (k, snd p + 1)
--   | otherwise = p
...
-- average xs = foldr1 (+) xs / fromIntegral (length xs)
...
winner scores player1 player2
  | scoreOfPlayer player1 >= scoreOfPlayer player2 = player1
  | otherwise = player2
  where
    scoreOfPlayer :: String -> Int
    scoreOfPlayer player = Map.findWithDefault 0 player scores
...
freqs xs = foldr (Map.alter incrementCount) Map.empty xs
  where
...
transfer from to amount bank
  | shouldTransfer = Map.adjust ((+) amount) to (Map.adjust (\x -> x - amount) from bank)
  | otherwise = bank
  where
    shouldTransfer
      | amount < 0 = False
      | Map.notMember from bank = False
      | Map.notMember to bank = False
      | Map.findWithDefault 0 from bank < amount = False
      | otherwise = True
...
maxIndex arr = fst $ foldr1 bigger $ Array.assocs arr
  where
    bigger (i1, a1) (i2, a2) = if a1 > a2 then (i1, a1) else (i2, a2)

-- maxIndex arr = fst $ findMaxIndex (Array.assocs arr)
--   where
--     findMaxIndex :: (Ix i, Ord a) => [(i, a)] -> (i, a)
--     findMaxIndex xs =
--       foldr1
--         ( \(i1, a1) (i2, a2) ->
--             if a1 > a2 then (i1, a1) else (i2, a2)
--         )
--         xs