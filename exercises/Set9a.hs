...
countValid ss = length $ filter id $ map validate ss
  where
    validate :: String -> Bool
    validate str = (str !! 2 == str !! 4) || (str !! 3 == str !! 5)
...
sumSuccess es = case rights es of
  [] -> Left "no data"
  rs -> Right (sum rs)

-- sumSuccess es = foldr f (Left "no data") es
--   where
--     f :: Either String Int -> Either String Int -> Either String Int
--     f (Right e) (Right acc) = Right (e + acc)
--     f (Right e) (Left _) = Right e
--     f (Left e) (Right acc) = Right acc
--     f (Left e) (Left acc) = Left acc
...
lock :: Lock -> Lock
lock (Locked l) = Locked l
lock (Opened l) = Locked l

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode _ (Locked l) = Locked l
changeCode str (Opened l) = Opened str
...
compose a b = [(x, z) | (x, y) <- a, Just z <- [lookup y b]]
...
permute p a = map snd $ sortBy (comparing fst) $ zip p a
