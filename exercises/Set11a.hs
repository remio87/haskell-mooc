...
readWords n = do
  str <- getLine
  rest <- readWords (n - 1)
  return $ sort (str : rest)

-- readWords :: Int -> IO [String]
-- readWords n = readWords' n (return [])

-- readWords' :: Int -> IO [String] -> IO [String]
-- readWords' 0 strs = strs
-- readWords' n strsIo = do
--   str <- getLine
--   strs <- readWords' (n - 1) strsIo
--   return $ sort (str : strs)
...
-- countdownPrint n = do
--   mapM_ (putStrLn . show) (reverse [0 .. n])
...
isums' n sum = do
  input <- getLine
  let updated = sum + (read input :: Int)
  print updated
  if n == 1
    then return updated
    else isums' (n - 1) updated
