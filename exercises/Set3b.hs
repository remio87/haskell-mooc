...
buildList _ 0 c = [c]
buildList a b c = a : buildList a (b - 1) c
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf (x : xs) = x : sumsOf' xs x

sumsOf' :: [Int] -> Int -> [Int]
sumsOf' [] _ = []
sumsOf' (x : xs) n = x + n : sumsOf' xs (x + n)
...
mymaximum bigger initial (x : xs) = mymaximum bigger next xs
  where
    next = if bigger initial x then initial else x
