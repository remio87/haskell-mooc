...
prettyPrint n coords = addNewlines $ snd $ mapAccumL trav (sort coords) allCells
  where
    trav :: [Coord] -> Coord -> ([Coord], Char)
    trav [] cur = ([], '.')
    trav (c : cs) cur
      | c == cur = (cs, 'Q')
      | otherwise = (c : cs, '.')
    allCells = [(x, y) | x <- [1 .. n], y <- [1 .. n]]
    addNewlines :: String -> String
    addNewlines [] = []
    addNewlines s = take n s ++ "\n" ++ addNewlines (drop n s)

-- prettyPrint n coords = concatMap ppLine rows
--   where
--     rows = [1 .. n]
--     sc = sortBy
--     ppCell c
...
-- prettyPrint' :: Size -> [Coord] -> Coord -> String
-- prettyPrint'
--   n
--   coords
--   (x, y) =
--     if x == n && y == n
--       then if elem (x, y) coords then "Q\n" else ".\n"
--       else
--         if y == n
--           then
--             if elem (x, y) coords
--               then "Q\n" ++ prettyPrint' n coords (nextRow (x, y))
--               else ".\n" ++ prettyPrint' n coords (nextRow (x, y))
--           else
--             if elem (x, y) coords
--               then "Q" ++ prettyPrint' n coords (nextCol (x, y))
--               else "." ++ prettyPrint' n coords (nextCol (x, y))
